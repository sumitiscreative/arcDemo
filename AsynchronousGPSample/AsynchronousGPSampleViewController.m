...
#define kBlueLine @""
#define kBlueLineStation @""
#define kRedLineStation @""
#define kRedLine @""
...
- (void)viewDidLoad {
    ...
    // code for dynamic map servers
    // http://129.63.163.68:6080/arcgis/rest/services/stlouis_0312/MapServer

    url = [NSURL URLWithString: @"http://129.63.163.68:6080/arcgis/rest/services/stlouis_0312/MapServer"];
//    AGSDynamicMapServiceLayer* layer = [AGSDynamicMapServiceLayer dynamicMapServiceLayerWithURL: url];
//    [self.mapView addMapLayer:layer withName:@"Service Layer"];
...
//XMin: -9289105.468992854
//YMin: 4317853.327422898
//XMax: -9222562.627573838
//YMax: 4343253.378223

    //Zooming to an initial envelope with the specified spatial reference of the map.
	AGSSpatialReference *sr = [AGSSpatialReference spatialReferenceWithWKID:102421];
	AGSEnvelope *env = [AGSEnvelope envelopeWithXmin:-9289105.468992854
                                                ymin:4317853.327422898
                                                xmax:-9222562.627573838
                                                ymax:4343253.378223
									spatialReference:sr];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    NSLog(@"blue %d ,red %d ,blue S %d ,red S %d",self.parameters.blueLine,
          self.parameters.redLine,
          self.parameters.blueLineStation,
          self.parameters.redLineStation);
    
    NSLog(@"day %@",self.parameters.dayOrNightIncident);
    
    
    if (self.parameters.blueLine) {
        [self addLayer:kBlueLine];
    }else if (self.parameters.redLine) {
        [self addLayer:kRedLine];
    }else if(self.parameters.blueLineStation) {
        [self addLayer:kBlueLineStation];
    }else if(self.parameters.redLineStation) {
        [self addLayer:kRedLineStation];
    }
...
#pragma mark Change Layers here

-(void)removeLayer:(NSString*)layerName{
    [self.mapView removeMapLayerWithName:layerName];
}

-(void)addLayer:(NSString*)layerName{

AGSDynamicMapServiceLayer* layer = [AGSDynamicMapServiceLayer dynamicMapServiceLayerWithURL: url];
[self.mapView addMapLayer:layer withName:layerName];
}
