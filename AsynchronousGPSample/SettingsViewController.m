...
- (void)viewDidLoad
{
    ...
    self.blueLineSwitch.tag = 1;
    self.blueLineStationSwitch.tag = 2;
        self.redLineSwitch.tag = 3;
        self.redLineStationSwitch.tag = 4;
    
    [self.blueLineSwitch setOn:self.parameters.blueLine];
    [self.blueLineStationSwitch setOn:self.parameters.blueLineStation];
    [self.redLineSwitch setOn:self.parameters.redLine];
    [self.redLineStationSwitch setOn:self.parameters.redLineStation];
    
    
    [self.blueLineSwitch addTarget:self
                      action:@selector(stateChanged:) forControlEvents:UIControlEventValueChanged];
    
    [self.blueLineStationSwitch addTarget:self
                            action:@selector(stateChanged:) forControlEvents:UIControlEventValueChanged];
    
    [self.redLineSwitch addTarget:self
                            action:@selector(stateChanged:) forControlEvents:UIControlEventValueChanged];
    
    [self.redLineStationSwitch addTarget:self
                            action:@selector(stateChanged:) forControlEvents:UIControlEventValueChanged];
...
-(void)stateChanged:(UISwitch*)switchState{
    ...
    if (switchState.tag == 1) {
        self.parameters.blueLine = [switchState isOn] ? YES : NO;
    }else if (switchState.tag == 2) {
        self.parameters.blueLineStation = [switchState isOn] ? YES : NO;
    }else if(switchState.tag == 3) {
        self.parameters.redLine = [switchState isOn] ? YES : NO;
    }else if(switchState.tag == 4) {
        self.parameters.redLineStation = [switchState isOn] ? YES : NO;
    }
    
}

-(BOOL)valueReturn:(UISwitch*)switchState{
    if ([switchState isOn]) {
        return YES;
    }else
        return NO;
